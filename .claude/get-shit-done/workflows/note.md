...
- **Global scope**: `D:/promptita/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/promptita/.claude/notes/`)
...
2. Glob `D:/promptita/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/promptita/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/promptita/.claude/notes/` — works in any directory
